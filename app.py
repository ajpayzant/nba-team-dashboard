# app.py — NBA Player + Team Dashboard (combined)
# - Two tabs: Player Scouting | Team Dashboard
# - Uses original code structure, minimal edits
# - Fix: resolve_team_id robust to full name / abbreviation
# - Player Projection: 0–1 sliders, normalized; CI 90/80/70 correct
# - Team tiles + roster tables stacked
...
# --- Static team maps (names, abbrevs, IDs) ---
def _build_static_maps():
    tdf = pd.DataFrame(static_teams.get_teams())
    tdf = tdf.rename(columns={"id":"TEAM_ID","full_name":"TEAM_NAME","abbreviation":"TEAM_ABBREVIATION"})
    tdf["TEAM_ID"] = tdf["TEAM_ID"].astype(int)

    by_full = dict(zip(tdf["TEAM_NAME"].astype(str), tdf["TEAM_ABBREVIATION"].astype(str)))
    id_by_full = dict(zip(tdf["TEAM_NAME"].astype(str), tdf["TEAM_ID"].astype(int)))
    id_by_abbr = dict(zip(tdf["TEAM_ABBREVIATION"].astype(str), tdf["TEAM_ID"].astype(int)))
    ...
    return by_full_cf, nick_cf, alias_up, id_by_full, id_by_abbr

BY_FULL_CF, NICK_CF, ABBR_ALIAS, TEAMID_BY_FULL, TEAMID_BY_ABBR = _build_static_maps()
...
def resolve_team_abbrev(team_name: str, team_ctx_row: pd.Series | None = None) -> str | None:
    ...
    if isinstance(team_name, str):
        ...
        # if already abbr, normalize aliases
        if 2 <= len(team_name.strip()) <= 4:
            return normalize_abbr(team_name)
    return None

def resolve_team_id(team_name_or_abbr: str, team_ctx_row: pd.Series | None = None) -> int | None:
    # from context row
    if team_ctx_row is not None and "TEAM_ID" in getattr(team_ctx_row, "index", []):
        try:
            return int(team_ctx_row["TEAM_ID"])
        except Exception:
            pass
    # full name
    if isinstance(team_name_or_abbr, str):
        cf = team_name_or_abbr.casefold().strip()
        if cf in BY_FULL_CF:
            full = None
            # recover real-cased full
            for k in TEAMID_BY_FULL.keys():
                if k.casefold() == cf:
                    full = k
                    break
            if full and full in TEAMID_BY_FULL:
                return TEAMID_BY_FULL[full]
        # abbreviation direct
        ab = normalize_abbr(team_name_or_abbr)
        if ab and ab in TEAMID_BY_ABBR:
            return TEAMID_BY_ABBR[ab]
    return None

# ----------------------- Caches: Players/Teams -----------------------
...
# ----------------------- DATA FETCHERS FOR TEAM DASHBOARD -----------------------
@st.cache_data(ttl=CACHE_HOURS*3600, show_spinner=False)
def get_teams_df():
...
def player_dashboard():
    st.header("Player Scouting Dashboard")
    ...
    opp_team_id = resolve_team_id(opponent, opp_row=None)
    ...
    # ---- Player Projection Summary (updated with 0–1 weights + normalized + correct CI) ----
    with st.expander("Player Projection Summary"):
        ...
        try:
            ...
            # vs Opponent snapshot (last 5 vs opponent, if any)
            ...
            st.markdown(
                ...
                *Notes:*  
                ...
                • If all weights are 0, the model defaults to **Recent = 1.00**.  
...
def team_dashboard():
    ...
    with st.sidebar:
        ...
        season = st.selectbox("Season (Team)", SEASONS, index=0, key="season_team")
        teams_df = get_teams_df()
        team_name = st.selectbox("Team", sorted(teams_df["TEAM_NAME"].tolist()), key="team_sel")
